...
tot_oc            = resumen["Monto_OC"].sum()   # Descarga_IConstruye filtrado a áridos
# Usar RecFac para Recibido; guardrail OC >= Recibido
if not df_recfac_f.empty:
    # Recepciones únicas (sin duplicar por doc de recepción)
    ...
    # (Total_OC de RecFac no se usa porque incluye partes no-áridas de OCs mixtas)
...
def _build_avance_charts(df_grp, label_col):
    ...
    def _ec_avance_chart(vals_ppto, vals_oc, vals_rec, x_unit, title):
        ...
        _fmt = (
            "function(params){"
            + f"var u='{_u}';"
            + "var n=params[0].name,p=0,o=0,r=0,mp='',mo='',mr='';"
            + "params.forEach(function(s){"
            + "if(s.seriesName==='Presupuestado'){p=s.value;mp=s.marker;}"
            + "else if(s.seriesName==='OC Efectivo'){o=s.value;mo=s.marker;}"
            + "else if(s.seriesName==='Recibido'){r=s.value;mr=s.marker;}"
            + "});"
            ...
            + "var h='<div style=\"font-size:11px;color:#F1F5F9;padding:6px 10px;min-width:220px;\">';"
            + "h+='<div style=\"margin-bottom:6px;font-weight:700;color:#38BDF8;\">'+n+'</div>';"
            + "h+='<table style=\"width:100%;border-collapse:collapse;\">';"
            + "h+='<tr><td style=\"color:#94A3B8;\">'+mp+' Presupuestado</td><td style=\"text-align:right;font-weight:600;padding-left:12px;\">'+fmt(p)+' '+u+'</td></tr>';"
            + "h+='<tr><td style=\"color:#94A3B8;\">'+mo+' OC Efectivo</td><td style=\"text-align:right;font-weight:600;padding-left:12px;\">'+fmt(o)+' '+u+'</td></tr>';"
            + "h+='<tr><td style=\"color:#94A3B8;\">'+mr+' Recibido</td><td style=\"text-align:right;font-weight:600;padding-left:12px;\">'+fmt(r)+' '+u+'</td></tr>';"
